...
class GeometricAttentionPooling(nn.Module):
    """
    融合了几何先验的 Attention Pooling 模块
    替代传统的 Max Pooling，用于 Ball Query 下的局部特征自适应聚合
    """
    def __init__(self, in_channels, hidden_dim=None):
        super().__init__()
        # 几何特征维度: 中心点(3) + 邻居点(3) + 相对坐标dp(3) + 距离dist(1) = 10
        self.geo_dim = 10 
        
        # 1. 【新增】：几何特征投影层 (将 10维 升维到 in_channels，与 f 对齐)
        self.geo_proj = nn.Sequential(
            nn.Conv2d(self.geo_dim, in_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(in_channels),
            nn.ReLU(inplace=True)
        )
        
        # 2. 注意力分数计算层 (因为前面是相加，这里输入的通道数只需要 in_channels)
        self.attn_mlp = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=1, bias=False),
            # 这里通常不需要 ReLU，直接输出 raw score 作为 Softmax 的输入
        )

    def forward(self, p_center, dp, fj):
        K = fj.shape[-1]
        
        # 1. 提取原始的 10 维几何先验
        p_center = p_center.transpose(1, 2).contiguous()
        p_center_expanded = p_center.unsqueeze(-1).expand(-1, -1, -1, K) 
        p_neighbor = p_center_expanded + dp                              
        dist = torch.sqrt(torch.sum(dp ** 2, dim=1, keepdim=True) + 1e-8) 
        
        raw_geo_feat = torch.cat([p_center_expanded, p_neighbor, dp, dist], dim=1) # (B, 10, N, K)
        
        # 2. 【核心修改】：将 10维几何特征 投影到 高维空间
        geo_embed = self.geo_proj(raw_geo_feat) # (B, C, N, K)
        
        # 3. 【执行你的思路】：将高维几何位置编码 与 语义特征 强行在相同维度下相加！
        combined_feat = geo_embed + fj # (B, C, N, K)
        
        # 4. 根据相加后的融合特征，计算注意力分数
        attn_scores = self.attn_mlp(combined_feat) # (B, C, N, K)
        
        # (可选) 处理 Ball Query 的 Padding 点，防止对距离为0的凑数点分配过多注意力
        is_padded = (torch.abs(dp).sum(dim=1, keepdim=True) < 1e-5)
        attn_scores = attn_scores.masked_fill(is_padded, -1e9)
        
        # 5. Softmax 计算权重并加权聚合
        attn_weights = F.softmax(attn_scores, dim=-1) # (B, C, N, K)
        f_pooled = torch.sum(attn_weights * fj, dim=-1) # (B, C, N)
...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        if self.reduction == 'attention':
            self.pool = GeometricAttentionPooling(in_channels=channels1[-1])
        else:
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if not is_head:
            ...
            self.grouper = create_grouper(group_args)
            self.reduction = kwargs.get('reduction', 'max').lower()
            if self.reduction == 'attention':
                self.pool = GeometricAttentionPooling(in_channels=channels[-1])
            else:
                self.pool = get_reduction_fn(self.reduction)
...
class PointMetaBaseEncoderV2(nn.Module):
