...
from ..layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
    create_grouper, furthest_point_sample, random_sample, three_interpolation, grouping_operation
import copy
...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        # ================= 模块 2：特征差分编码器 (对应图中的 f_j - f_i 支路) =================
        # 注意：这里使用 2D 卷积来处理带有邻居维度 K 的特征张量
        convs_df = []
        for i in range(len(channels) - 1):
            convs_df.append(create_convblock2d(channels[i], channels[i + 1],
                                            norm_args=norm_args,
                                            act_args=None if i == (len(channels) - 2) and not last_act else act_args,
                                            **conv_args))
        self.convs_df = nn.Sequential(*convs_df)
    ...
    def forward(self, pf) -> torch.Tensor:
        ...
        df = fj - f.unsqueeze(-1)
        df_encoded = self.convs_df(df)
        f_fused = self.pool(df_encoded + pe) #+ f_raw
        f = f + f_fused
...
class InvResMLP(nn.Module):
    ...
    def forward(self, pf):
        ...
        dp, fj =self.grouper(p, p, f)
        pe = self.rel_pos(dp)
        p, f, dp, fj, pe = self.layer([p, f, dp, fj, pe])
        return p, f


@MODELS.register_module()
class HPENetV2Encoderv2(nn.Module):
